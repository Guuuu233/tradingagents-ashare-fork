...
_TUSHARE_REPURCHASE_API = "repurchase"
...
class CnAkshareProvider(BaseMarketDataProvider):
    ...
    def _fetch_tushare_repurchase(
        ...
        """Fetch structured repurchase (股票回购) collateral records via Tushare transport.

        Enforces contract (C-05 Slice 3 / DAV-642):
        1. PIT boundary guard: rows with ann_date > as_of are discarded. Strictly forbids
        ...
        4. canonical_event_id is ALWAYS None (strictly forbids inventing cninfo ID).
        5. Reuses existing _tushare_post without creating a new HTTP client.
        6. Fields extracted by name: ts_code, ann_date, end_date, proc, exp_date, vol, amount, high_limit, low_limit.
        7. Preserves earlier stages when proc='完成' announcement date is in future (prevents completion bias).

        Returns:
            (records, error_str, failure_category)
            - records: list of dicts containing repurchase collateral fields by name,
                       with canonical_event_id=None, or [] on failure/empty.
            - error_str: typed error string formatted as tushare.repurchase:<category>(detail)
            - failure_category: category slug (provider_failure, collateral_empty,
                                schema_drift, json_shape, validation, etc.)
        """
        # 1. PIT as_of normalization & validation
        ...
        if as_of:
            ...
            if not norm_as_of:
                return (
                    [],
                    self._tushare_error(_TUSHARE_REPURCHASE_API, "validation", "as_of"),
                    "validation",
                )
        ...
        if not token:
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "provider_failure", "token_missing"),
                "provider_failure",
            )
        ...
        try:
            ...
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "validation", "symbol"),
                "validation",
            )
        ...
        if ann_date:
            ...
            if not norm_ann:
                return (
                    [],
                    self._tushare_error(_TUSHARE_REPURCHASE_API, "validation", "ann_date"),
                    "validation",
                )
            params["ann_date"] = norm_ann.replace("-", "")
        if start_date:
            ...
            if not norm_start:
                return (
                    [],
                    self._tushare_error(_TUSHARE_REPURCHASE_API, "validation", "start_date"),
                    "validation",
                )
            params["start_date"] = norm_start.replace("-", "")
        if end_date:
            ...
            if not norm_end:
                return (
                    [],
                    self._tushare_error(_TUSHARE_REPURCHASE_API, "validation", "end_date"),
                    "validation",
                )
            params["end_date"] = norm_end.replace("-", "")
        ...
        if error:
            if category == "permission_denied" or "403" in str(error) or "403" in str(category):
                return (
                    [],
                    self._tushare_error(_TUSHARE_REPURCHASE_API, "provider_failure", "403_forbidden"),
                    "provider_failure",
                )
            return [], error, category
        ...
        if not isinstance(payload, dict):
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "json_shape"),
                "json_shape",
            )
        ...
        try:
            ...
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "api_code_invalid"),
                "api_code_invalid",
            )
        if code_value != 0:
            ...
            if cat == "permission_denied" or is_perm_or_token:
                return (
                    [],
                    self._tushare_error(
                        _TUSHARE_REPURCHASE_API, "provider_failure", f"permission_denied:code={code}"
                    ),
                    "provider_failure",
                )
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, cat, f"code={code}"),
                cat,
            )
        ...
        if "data" not in payload:
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "json_shape", "data_missing"),
                "json_shape",
            )
        data = payload.get("data")
        if data is None:
        ...
        if not isinstance(data, dict):
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "json_shape", "data_not_object"),
                "json_shape",
            )
        fields = data.get("fields")
        items = data.get("items")
        if not isinstance(fields, (list, tuple)):
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "json_shape", "fields_not_list"),
                "json_shape",
            )
        if not isinstance(items, (list, tuple)):
            return (
                [],
                self._tushare_error(_TUSHARE_REPURCHASE_API, "json_shape", "items_not_list"),
                "json_shape",
            )
        ...
        if "ann_date" not in field_names:
            return (
                [],
                self._tushare_error(
                    _TUSHARE_REPURCHASE_API, "schema_drift", "missing_field:ann_date"
                ),
                "schema_drift",
            )
        if not items:
        ...
        for item in items:
            if isinstance(item, dict):
                ...
                return (
                    [],
                    self._tushare_error(
                        _TUSHARE_REPURCHASE_API, "schema_drift", "malformed_row"
                    ),
                    "schema_drift",
                )
            ...
            if not norm_ann_date:
                return (
                    [],
                    self._tushare_error(
                        _TUSHARE_REPURCHASE_API, "schema_drift", "invalid_date:ann_date"
                    ),
                    "schema_drift",
                )

            # PIT boundary check: discard row if ann_date > as_of
            # Strictly forbidden: never truncate with end_date or exp_date!
            if norm_as_of and norm_ann_date > norm_as_of:
                continue
            ...
            row_end_date = str(raw_row.get("end_date") or "").strip()
            row_proc = str(raw_row.get("proc") or "").strip()
            row_exp_date = str(raw_row.get("exp_date") or "").strip()
            vol = raw_row.get("vol")
            amount = raw_row.get("amount")
            high_limit = raw_row.get("high_limit")
            low_limit = raw_row.get("low_limit")

            collateral_id = f"tushare:repurchase:{row_ts_code or ts_code}:{norm_ann_date}"
            if row_end_date:
                collateral_id = f"{collateral_id}:{row_end_date}"
            if row_proc:
                collateral_id = f"{collateral_id}:{row_proc}"

            # Contract 4: canonical_event_id MUST be None (strictly forbids inventing cninfo ID)
            record = {
                "canonical_event_id": None,
                "collateral_id": collateral_id,
