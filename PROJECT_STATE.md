...
| 数据库写入保护 | 部署前后 reports 计数未变；只读评估输入为部署前 SQLite backup，不是运行中的库 |
| 部署回退点 | 旧发布 worktree `/private/tmp/ta-release-4f1a1aa3-20260914` 保留，供受控回退核验 |
...
- DAV-887（`4e6266b171b8431c1b979f7d70f0c075370c9b9d`）：TrackingBoard/Portfolio 的 raw direction 显示源代码已接入 `localizeDirection`；前端 live build/UI smoke 尚未单独核验。
...
| P1 | 裁决者一手证据 | 设计项未开工 | 先冻结 bounded evidence summary、截断和来源规则，再为 trader/risk_manager 建实施卡；不得直接塞入完整七份报告。 |
| P1 | 财务披露日 PIT | 已知边界 | 逐票公告日来源、as-of 规则、缺失/未来日期语义和回归 fixture 先定规格，再派窄修。 |
| P1 | 前端产品验收 | 源码修复已合入，运行证据缺 | 在不改后端语义的前提下完成 frontend test/build 和真实入口 smoke。 |
