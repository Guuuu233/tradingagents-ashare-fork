...
## 当前状态头部（2026-09-27 01:30 总控更新；开工前必须重新回读，D-005）

> 项目总图见 `docs/PROJECT_PANORAMA.md`（快照，给全体成员看的概览）。

| 项 | 值 |
|---|---|
| 生产代码 SHA | `92863063e7def13bc52b0fd6f576cf4b384ee779`（09-27 00:2x，含 DAV-1305 档位同步、1314 用量账本、1315 luna 重复机读块隔离、1325 devin 温度、1330 账本角色/档位归因、1331 devin 并发上限 4；**价格返修与 E-04 返修均开启**）。回退为 `releases/9ff06213` |
| 主干 | 以 `git ls-remote origin codex/dav-4-p2a-trunk` 回读为准。09-27 01:30 主干 = 生产 `92863063` 加本次台账提交。**只有总控或按总控签字 SHA 快进的总工可以推主干（D-048）** |
| 待发布提交 | 无 |
...
**09-27 运行事实：**
- David 的 15 个角色目前绑定 `devin/swe-2`。swe-2 试跑（报告 4ba41316）验收通过，一份双档分析约 25 分钟（并发上限 4），约 97 万 token。swe-2 样本与 gemini、luna 样本分开统计。
- luna 禁令：stream_options 实证完成前，不得把角色切回 gpt-6-luna（DAV-1314）。
- 用量账本（DAV-1314/1330）已可按角色、按档位拆账。gemini 一份双档分析约 130 万 token，缓存命中约 19%。

**09-26 第一阶段剩余工作（D-046，按顺序）：**
1. ~~DAV-1310～1312 诊断~~：已签收（D-047）；
2. **当前施工（D-049 调整）：DAV-1338（urgent，守卫把证据簇当同一事件、掏空采纳账本）→ DAV-1321（价格提取器精度，施工中）**；DAV-1322 已合入（ae762eed，待随批部署）；DAV-1320 暂缓，待 1338 之后重测；1338、1321 合入后须总控按 D-037 单独放行；
3. 概率输出契约；
4. 测量。
