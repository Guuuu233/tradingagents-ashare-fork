"""Tests for V-03 Offline Replay Harness & Snapshot Protocol (DAV-861 / DAV-802 / DAV-804).

Comprehensive coverage for 16 Red Team Scenarios RT-1 to RT-16:
- RT-1: 生产库与 SQLite 副本隔离 (只读副本运行，生产库 hash / 计数零变化，quick_check 通过)
- RT-2: 多账号 + failed/pending/running 混库 (仅 target_user_id 且仅 completed 入池与统计)
- RT-3: 25 字段离线审计表 (每行字段齐备，缺失来源显式 typed gap，不伪造默认值，严禁 carry-forward)
- RT-4: 三段 OOS 边界 (DEV <= 2025-12-31, HISTORICAL_OOS 2026-01-01~09-08, FORWARD_OOS >= 2026-09-09 零泄漏)
- RT-5: 六只回归标的 (歌尔/富联/蓝思/美的/隆基/爱尔 永久 sample_role=regression，不进 OOS 汇总)
- RT-6: typed-missing (蓝思 6 月缺口类：return=NULL，进 coverage，不进 return 分母，不静默 drop)
- RT-7: T+1 Open 与不可执行入场 (次日停牌/涨跌停/缺价标 untradable，严禁假装以 Open 成交，零前视)
- RT-8: 成本与沪深300基准 (冻结佣金含规费+过户+印花+5bps滑点，同窗超额收益 Alpha 可复核)
- RT-9: 无概率校准 / 无组合规则 (不输出 confidence->probability，不输出 Sharpe 或最大回撤)
- RT-10: 复制消融 (Repetition: 同质复制不增独立票，独立事实不误杀，不以关键词频充票)
- RT-11: 顺序消融 (Order: Canonical/Reversed/Seeded-Shuffled 共享同一快照与样本集，seed=42 可复放)
- RT-12: 缺口消融 (Gap: mask 关键字段显式产生 typed gap，不填 0、不填默认、不 carry-forward、不删样本)
- RT-13: 命题消融 (Proposition: 无命题时判定 not_checked / typed gap，禁止伪造挑战/解决信号)
- RT-14: 同事件/重叠标签 (Purging/Embargo: 同标的重叠持仓区间样本剔除，留审计痕迹，防时间泄漏)
- RT-15: 变体同快照 (所有消融变体共享相同 cutoff、资格、成本模型与 snapshot_hash)
- RT-16: 当前没有 FORWARD_OOS (如实报告 0 与未纳原因，严禁将历史样本改名充作 forward)
"""
...
def mock_price_provider(sample_trade_dates):
    ...
    # 5. Lens Technology 300433.SZ (Regression symbol, and June gap: exit date bar missing)
    provider.add_bar(
        "300433.SZ",
        DailyBar(
            date="2026-06-01",
    ...
    # exit date 2026-06-09 intentionally NOT added to simulate typed_missing exit gap
...
# ===========================================================================
# Supplementary & Legacy Regression Tests
# ===========================================================================


def test_legacy_collision_stock_merged_canonical(mock_price_provider):
    """Verify 000001 / 000001.SZ collision symbols are merged into single canonical entity."""
    engine = V03ReturnMeasureEngine(price_provider=mock_price_provider, hold_days=5)
    reports = [
        {"id": "rep_col_01", "symbol": "000001", "trade_date": "2026-03-02", "decision": "BUY"},
        {"id": "rep_col_02", "symbol": "000001.SZ", "trade_date": "2026-03-02", "decision": "BUY"},
    ]
    res = engine.measure_dataset(reports)
    # Both canonicalized to 000001.SZ
    assert res.records[0].symbol_canonical == "000001.SZ"
    assert res.records[1].symbol_canonical == "000001.SZ"


def test_legacy_stock_pool_filtering_excluded_and_counted(mock_price_provider):
    """Verify ST, BSE, and new listings (<60d) are excluded from pool and counted."""
    engine = V03ReturnMeasureEngine(price_provider=mock_price_provider, hold_days=5)
    reports = [
        {"id": "r_st", "symbol": "000002.SZ", "trade_date": "2026-03-02"},  # ST
        {"id": "r_bse", "symbol": "830001", "trade_date": "2026-03-02"},  # BSE
        {"id": "r_new", "symbol": "000003.SZ", "trade_date": "2026-08-10"},  # <60d
    ]
    res = engine.measure_dataset(reports)
    assert res.all_metrics.excluded_pool_count == 3
    assert res.all_metrics.in_pool_count == 0
...
def test_rt_s4_david_account_clean_population_counts(mock_price_provider):
    """RT-S4: 该账号在基准日 2026-09-08 completed=231、评估候选≈217、DEV=FORWARD=0."""
    ...
    # 1. Query counts under frozen cutoff date 2026-09-08
    counts = V03ReturnMeasureEngine.get_user_report_counts(
        prod_db_path, target_user_id=DEFAULT_TARGET_USER_ID, cutoff_date=DEFAULT_HISTORICAL_CUTOFF_DATE
    )
    assert counts["total"] == 317
    assert counts["completed"] == 231
    assert counts["failed"] == 86

    # 2. Query live counts without cutoff
    counts_live = V03ReturnMeasureEngine.get_user_report_counts(
        prod_db_path, target_user_id=DEFAULT_TARGET_USER_ID, cutoff_date=None
    )
    assert counts_live["total"] >= 317
    assert counts_live["completed"] >= 231

    # 3. Load David completed reports under frozen cutoff
    ...
    engine = V03ReturnMeasureEngine(
        ...
        status_filter=DEFAULT_STATUS_FILTER,
        target_user_stats=counts,
        cutoff_datetime=DEFAULT_HISTORICAL_CUTOFF_DATETIME,
        requested_as_of=DEFAULT_HISTORICAL_CUTOFF_DATE,
    )
    res = engine.measure_dataset(reports)
