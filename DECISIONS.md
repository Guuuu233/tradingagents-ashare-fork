...
### D-042：DAV-1235 规格修正、准备线与前端清单的裁决（有效，2026-09-24）

- **DAV-1228 准备线**：squash 提交 `0833925e5cbae5f14275603045ae1367e3db8f10` 准予并入。
  - 核验：父提交为 `88442fc`；只含 `work/prep-c-f1-phase-h/` 下 8 个文件；4 个 md 与 `probe2_tushare.json` 与原分支 `15dfbd4` 逐字节相同；probe1/3/4 已去除样本行；`git diff --check` 通过；凭据扫描无命中。
  - 两个残留字段（probe1 的 `example.code` 与 `n_versions`、`p1b_20101231.sample_dates`）是证券代码、计数和公告日期，属于元信息，保留。
  - 并入后删除原分支 `agent/agent/665f3af82f25` 和 squash 分支。
- **DAV-1235 候选 `a4ce75c1ec37c61ab6620151cadcaf777317aaab` 不予合入，按修正后的规格返工。**
  - 原因是总控规格写得过宽，不是实现错误。候选逐字实现了规格，但冻结语料从 6/50 降到 0/50，原来通过的 6 份全部变为不合格，而这 6 份的豁免都是正当的估值公式，例如「给予 18 倍 PE，对应市值约 504 亿元，折合股价 38~40 元」。
  - 问题一：P1 要求估值词与计算关联「同一子句」，但真实写法常用逗号或括号把公式拆开。
  - 问题二：P2 的否决词表含「支撑、压力、阻力、平台、箱体、最高、最低」，这些词同样用来描述估值底线（「估值支撑位」「历史 PB 最低点」）。
  - 问题三：「1.8~2.0 倍」中的 1.8 被登记成价格。倍数尾部的正则不认带小数的区间写法。
  - 另有两项交付要求：
    - 完整重算输出 `baseline.json`、`candidate.json`（各约 3.5 MB）不入库，只提交变化清单、汇总和它们的 SHA256；
    - `_baseline/price_ref_registry_a181e4a.py` 是生产代码的整份拷贝，不入库，重算脚本改为运行时用 `git show a181e4a:<路径>` 取基线。
  - 修正后的规格写在 DAV-1235 卡片评论里（P1′、P2′、P5′），其余条款不变。
- **DAV-1236 前端清单**（交付 `5331fd4`，以文档形式并入主干）逐项裁决：
  - 放行：a09ca2c 的前端部分、8325943、9590f4c、85a32f7、4e6266b、8ccecb8、331a322、49f1e87。
  - 排除 e368362：聊天入口强制 `v2_debate_enabled=true`，属于 D-037 冻结的生成链改造。
  - 排除 c62ff8e 的聊天选档控件：后端 `ChatCompletionRequest` 没有 `horizons` 字段，控件上线后不起作用，会误导用户。
  - 更正清单中的一处判断：49f1e87 改动的 research_manager 提示词属于后端代码，已随 `a181e4a` 上线。前端发布只替换 `frontend/dist`，不会“连带”提示词变更。
  - 执行方式：由一张前端发布准备卡在主干上删除这两处改动并补测试；前端随 DAV-1235 之后的下一次发布一起上线，走同一个两段式发布门，不单独部署。

- **2026-09-24 补充（收口）**：
  - DAV-1238 `8110f61` 与 DAV-1235 `e6301baf437e3ff18c7ce95bd11c890364df8565` 先后快进入主干，均经总控独立核验后签字。
  - DAV-1235 共返工两次，两次都因总控规格过宽：
    - 第一次，「同一子句」要求加上宽泛的否决词表，冻结语料从 6/50 变成 0/50；
    - 第二次，允许「紧邻的前一子句」提供计算关联，把颈线价、现价、价格区间误判为估值推算。
  - 终版口径：
    - (a) 估值模型词须在数值前 60 字以内，且在同一句；
    - (b) 计算关联词须在数值所在子句内、数值之前；
    - 技术指标硬否决；现价、收盘等行情词只在直接修饰该数值时否决；
    - 倍数（含小数区间）不登记为价格。
  - 终版结果：冻结语料 5/50；4390ddfd 的 derived_estimate 只剩 1274，其余 34 条违规都是模型自拟的价位。
  - 规则：今后总控发布含词表或正则的规格前，先在冻结语料的真实句子上试算。
  - 统一发布门另立卡，本次同批上线 DAV-1235 与 DAV-1238：
    - 前端在发布目录内对精确提交号构建；
    - 启动环境加 `APP_VERSION=0.6.0`。
  - 已知情况，暂不处理：服务每次启动都会向上游作者的站点 `app.510168.xyz` 上报一次版本号和随机数（`_report_version_stats`，上游遗留），不含用户数据。是否关闭，由 David 决定。

- **2026-09-24 发布（DAV-1242）**：
  - 生产 `a181e4a` → `b0ceff3029e3e5f786598d1bdb9bb37f62450f65`，13:25 切换，总控签收。
  - 上线内容：DAV-1235 价格引用收紧、DAV-1243 关闭启动时向上游的版本上报（David 拍板）、DAV-1238 等前端改动。
  - D-041 的运行规范在本次按以下三点补充：
    1. 前端在发布目录内对精确提交号构建，并回读 bundle 名；
    2. 启动白名单改为 14 项，新增 `APP_VERSION=0.6.0`；
    3. 启动后核验没有发往上游站点的请求。
  - 回退源为上一发布目录。

### D-043：生成侧价格标注的两轮试验，以及冻结语料指标口径的更正（有效，2026-09-24）

- **口径更正（最重要）：** 此前冻结语料上的「gate 通过数」都把 ABSTAIN 的 run 算作 clean，包括 W4 的 6/50、DAV-1224 的 6/50、DAV-1235 的 5/50、DAV-1246 的 1/20、DAV-1249 的 16/20。
  - ABSTAIN 的 run，其投资计划、交易计划和最终决策只是几十到几百字的占位文本，本来就不含价格，自然不会违规，但按 D-009 也不进入 H1b。
  - 以基线 5/50 为例：其中 4 个是 ABSTAIN、1 个是 VALID/WAIT，D-009 合格数为 0。
  - 从本条起，冻结语料的主指标改为**合格 clean**，同时满足三个条件：
    - VALID；
    - gate 前的动作属于 BUY、SELL 或 HOLD（取 `investment_debate_state.manager_verdict.trade_action`）；
    - gate 通过。
  - 「gate 通过数」只作辅助指标。
  - 生产 H1b 漏斗（clean 12）先按 D-009 筛选，不受这一缺陷影响。
- **DAV-1246（提示词注入价位表与书写规则）：无效，B 部分不合入。**
  - 通过数 1/20，每次运行违规 10.35；基线为每次 10.14。
  - A 部分（影线长度不登记、牛熊线别名）以 `b63c35b` 单独合入。
  - 试验 harness 没有把 pool 挂到 finalize；总控带 pool 离线重算后，结论不变。
- **DAV-1249（代码校验 + 定向返修一次）：机制有效，但第一版不合入，按 v2 返工。**
  - 违规从每次 9.85 降到 1.35。
  - 合格 clean：DAV-1249 为 1/20，DAV-1246 为 0/20，基线为 0/50。样本太少，还不能下结论。
  - 发现三个缺陷，都已要求在 v2 中修正：
    1. 保护签名比对的是整个机读块，包括理由文本和价格，而不是结论字段；
    2. 返修调用没有带上角色原有的系统提示和上下文；
    3. 返修稿没有持久化。
  - 6 次研究经理采用返修稿的 run 中，有 4 次落在 ABSTAIN，是否存在因果关系待 v2 验证。
- **DAV-1250：** `test_rt_s4` 改用合成夹具，RT-FULL 的固定既有失败清零（`e6b5330`）。
- **规则：** 总控今后发布含词表或正则的规格前，先在冻结语料的真实句子上试算。评估生成侧改动时，必须设同代码的对照组，并按合格 clean 口径汇报。
