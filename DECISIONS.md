...
### D-044：开启价格返修，统一发布门 DAV-1258（有效，2026-09-25）

- **合入主干：**
  - DAV-1249 v2 `8bfa1fb`：逐角色价格检查加定向返修一次，由开关 `TA_PRICE_REF_REVISION_ENABLED` 控制；
  - DAV-1255 `f075124`：可执行价位抽取器跳过日期、指标周期和金额的误读。
  - 两者都由总控独立复算，并签字。
- **证据：** 同一代码、同一冻结输入（DAV-1222 的 10 个样本），两组各 3 次、交替执行，60 个 state 按 f075124 离线重算：
  - 合格 clean（D-043 口径）：开启返修 **4/30**，关闭 **0/30**；
  - gate 通过：22/30 对 3/30；
  - VALID：13/30 对 11/30；
  - 违规合计：55 对 290；
  - 「改变结论的返修稿被采用」为 0；
  - s06 及两个坏锚全部被拦下。
  - 新增通过的样本都经过总控逐条审计：桥接是严格字段级；豁免都是 PB 或 PE 公式；可执行价位都是合法的 vendor_qfq，其中部分通过技术报告 backref 取得，属于现行契约允许的通道。
- **裁决：** 在下一次发布中，生产环境开启返修，即 `TA_PRICE_REF_REVISION_ENABLED=1`，白名单改为 15 项。这是 D-037 下由总控单独放行的生成链改造。
- **上线后监控：**
  - 总控逐份审计上线后的前 20 份真实报告；
  - 出现以下任一情况即关闭返修：假 clean、改变结论的返修稿被采用、VALID 比例明显下降、返修导致的失败或超时。
- **仍需解决的问题：**
  - ABSTAIN 约占 60–70%，主因是研究经理一致性硬门。返修无法挽救这部分样本，下一步需要单独诊断。
  - 技术报告中模型自拟的价位，经 backref 取得 vendor_qfq，这是现行契约的已知口径。
  - DAV-1257：归档审计脚本需要适配新的抽取器，优先级低。

### D-045：ABSTAIN 诊断、硬门确定性修复与研究经理 E-04 返修（有效，2026-09-25）

- **诊断（DAV-1261 `9056a17`）：**
  - 账户在 2026-08-26 之后的 completed 报告中，ABSTAIN 约占 65%。首因主要是经理一致性硬门（其中 E-04 类最多，其次是漏裁），以及资金流守卫。
  - E-04 的命中绝大多数是真实的裸断言，资金流守卫拦下的也都是真实冲突，两者都不放宽。
  - 但诊断发现三处确定性缺陷。
- **DAV-1264 `a909d43`：**
  - F1：告警尾缀不再自指命中。剥离锚定到文末，并与生成格式共用同一模板，防止模型仿写标记来逃避检查。
  - F2：确认状态改用 double_count_guard 裁剪后的最终账本。
  - F3：已定价标注的豁免中，连接词改为可选。
  - 在 272 份历史 ABSTAIN 上的直接效果：+25 VALID，其中合格 13；没有反向变化。
- **DAV-1267 `3f34db5`：** 研究经理在失败项全部为 E-04 时，定向返修一次。
  - 与价格返修合并为一次调用。
  - 新增同义词断言检查；🟡-1 修正后，该检查只在包含 E-04 段时执行。
  - 独立开关 `TA_E04_REVISION_ENABLED`，必须与 `TA_PRICE_REF_REVISION_ENABLED` 同时设置才生效。
  - 受控试验（两组各 10×3）：E-04 返修采用 10/11，采用后全部转为 VALID，其中合格 3；逐条人工审计，同义替换逃逸为 0，改变结论的返修稿被采用为 0。
  - 两组的合格数为 5 对 6，差距在 run 间方差范围内。以同一 run 的反事实为准。
  - 仅开价格返修的对照组，合格为 6/30，高于 DAV-1249 时的 4/30。
- **裁决：** 经 DAV-1270 发布上线，生产开启 E-04 返修，启动白名单改为 16 项。
- **上线后监控：** 沿用 D-044，并增加一项：凡采用 E-04 返修的报告，由总控逐条审计改写句。出现同义替换逃逸或改变结论的采用，立即回退。

### D-046：第一阶段剩余工作的顺序与口径；H1b 测量通道断开；历史样本批补登记（有效，2026-09-26）

- **背景：** David 09-26 指出，第二阶段（F1 冻结经验包协议 v1.0.1）开始之前，第一阶段必须先做完。总控按 `docs/PROJECT_PANORAMA.md` 与本文件路线逐项对照，并在生产库只读备份上重算指标。ChatGPT 独立复核后提出修正，本条已吸收。
- **新发现：H1b 测量通道结构性断开。**
  - H1b 只收 `protocol_version=v2_structured` 的报告；
  - 生产自 09-19 起的 157 份 completed 报告全部为 `v1_legacy`（`v2_debate_enabled` 关闭，D-042）；
  - 当前 cohort 的 H1b 样本量为 0/60。
- **09-25 两项返修上线后的指标（按档位）：**
  - 历史样本批 128 档：ABSTAIN 37%（其中 44/47 为研究经理自洽硬门）；价格门拦截 31%；合格 clean 23%；带概率 5%。
  - 日常真实分析仅 16 档，数量不足以下结论。
  - 两组不合并：日常真实分析是 readiness 的主样本，历史回放只用于故障诊断。
- **裁决：第一阶段剩余工作的顺序。**
  1. **H1b 资格诊断**（DAV-1310）。先确认 H1b 需要的最小语义字段，再判断 v1_legacy 能否等价提供；能提供就改资格解析层；只有确实缺信息时，才把“生产开启 v2”作为受控的生成链改造单独验证。不得为了适配测量去改被测系统。
  2. **两道门的精确率审计**，并行进行：研究经理自洽硬门（DAV-1311），价格门（DAV-1312）。指标是误拦率，不是拦截率。“不得降门”（ROADMAP §4）不变。
  3. **概率输出契约**：生成侧改造，D-037 下单独放行。
     - 现有 probability 字段的含义是“主周期绝对上涨概率”，而 F1 需要“T+10 行业相对概率”（DAV-1226 已指出），两者不是同一个量。
     - 第一阶段的契约设计要让阶段 P 的概率端点复用同一套解析、刻度与无效输出规则。
  4. **方差政策不单独设计**，直接并入协议阶段 R（A1/B1/A2/B2）。DAV-1222 的“同输入 5 次，动作全一致 2/10”作为执行阶段 R 的实证依据。
- **B2+B3 的 24 条重放（原路线第 6 步）：** 退出正式测量池，保留为“修复前诊断语料”，用作以后修复效果的回归对照。不删除，不计入新指标。
- **第一阶段出口标准：** 总控先前提出的数字（clean >50%、概率 >80%）没有推导来源，撤回。
  - 改为分两套，都由下游倒推：
    - A. 产品链路 readiness：门的误拦率，以及按 H1b 门槛和攒样周期倒推的合格产出率；
    - B. 测量 readiness：按阶段 R 的机会数与格式无效率上限倒推的概率有效率，以及输入冻结和模型指纹可追踪。
  - 具体数值在 DAV-1310～1312 完成后由总控提出，David 确认后冻结。
- **补登记（治理）：** 09-26 的历史基准样本批属于“批量真实分析”冻结项，应 David 当面要求由总控放行，事前没有登记，现补登记。
  - 账户固定为 `429163f7-…`，脚本登录后断言 user.id。
  - gemini-3.8 共 58 份；gpt-6-luna 约 32 份，因 gemini 额度耗尽，由 David 换模型后续跑，两组分开统计。
  - 冻结清单为 `work/hist-bench-20260926/frozen-20260926.json`，含每份 result_data 的 sha256。
  - 定位是探索性体检，不作为效果证据。
  - 已发现的问题：打分口径已改为 T+1 开盘；DAV-1305、1308、1309。

### D-047：五张交付的签收与整合方式；两道门审计结论与后续施工（有效，2026-09-26）

- **整合方式（总控特批，只适用于同一起跑线、互不重叠的交付）：**
  - 五个候选的父提交都是 `5e475dd`。
  - DAV-1315 `247d3ab` 直接快进。
  - DAV-1299 `75f63d4`、DAV-1310 `daa0218`、DAV-1312 `9edbb71` 由总控 cherry-pick 接到主干，逐个核对 patch-id，与复审版本一致。
  - DAV-1311 `40408049` 同样 cherry-pick，但按 D-040 剔除报告原文卷宗（`dossiers/`、`render_all.txt`、`e04_ctx.txt`，共约 3.4 万行模型输出与供应商数据片段），可用入库的 `extract.py` 从库重建。其余文件逐字节一致。
  - 整合后主干全量回归：5692 通过，0 失败，按 DAV-979 口径 deselect。
  - 这种方式免去了逐张重定基与重新复审；判断依据是内容与复审版本逐字节一致。
- **DAV-1310（H1b 资格）：** v1_legacy 已经提供 H1b 所需字段，断点在双档未拆包和资格判据写死 v2。**不开启 v2**，改资格解析层（DAV-1322）。总控补充：DAV-1310 的计数把顶层也算作一个单元，而顶层是主档切片的复制，DAV-1322 必须去掉；cohort 键加入档位。
- **DAV-1311（研究经理硬门）：**
  - 按冻结标准，误拦 29/47。总控抽查 10 例：8 例同意；1 例审计过严（af25901e:short 的 INV-3，'0.87' 实际已列入 excluded_evidence，应判误拦）；1 例与同类判定口径不一致（9b70d0a1 与 f4f0cac6 对“公开时长”是否构成定价依据的判断不同），需要统一。
  - 总控追查发现一个确定性缺陷：一致性检查跑在防重复计入守卫裁剪之前的账本上，裁剪后没有重算。“全额采纳含未核实证据”的 30 次命中里，24 次指向已被守卫剔除的论点 → DAV-1320。
  - **口径裁决：** 账本归类错误（被否决论点放进部分采纳；全额采纳后又剔除原子，却没改列为部分采纳）属于真实缺陷，因为 H1b 与信用加权都读这份账本。处理方向是“在结论只消费已核实子事实时，确定性地把账本改正”，而不是把硬门降级为告警。这类规则要等 DAV-1320 合入后，按剩余误拦重新测算，再与 E-04 的误拦/漏拦（DAV-1217）一起立卡。
- **DAV-1312（价格门）：** 误拦 42/46，其中 90.8% 的违规是提取器把非价格数字登记成价格 → DAV-1321。DAV-1308 可消除 0 档，DAV-1309 可消除 0 档（只消除 4 条违规），两卡维持低优先级。
- **DAV-1315：** luna 下被拒收那次尝试的原文带进 result_data，导致整份报告入库失败；修复为入库前隔离被拒收尝试的机读块，校验不放宽。总控验证：风控辩论路径遇到重复块同样不会导致入库失败。
- **DAV-1314：** 选 B，先施工，模型侧实证放在 luna 额度恢复后补做。开发者误把旧提交 `d32d428` 当成主干 tip，已纠正。
- **部署：** 产品代码相对生产 `3788527` 的差异为 DAV-1305 `70961ba` 与 DAV-1315 `247d3ab`，签准予部署（见 DAV-1315 签字评论）。

### D-048：主干推送权限；DAV-1330 直推事件的处置（有效，2026-09-27）

- **事件：** 2026-09-26 23:46，资深开发1交付 DAV-1330 时执行 `git push origin HEAD:codex/dav-4-p2a-trunk`，把候选 `792ecac0` 直接快进进了主干，事前没有经过复审，也没有总控签字。原因有两层：派工评论里写的“推远端后报完整 SHA”被理解成了推主干；远端对主干没有任何保护。调度助手把这次直推当作正常交付核收了。生产没有受影响。
- **处置：** 选追认路线。主干强推回退属于改写已发布的历史，是须 David 单独授权的红线，没有采用。DAV-1332 复审通过，总控复核（76 个相关测试通过）后，补签“准予合入（追认）”。若当时复审不通过，会用 `git revert` 做正向回滚。
- **规矩：**
  1. 开发、审核员、调度助手一律不得推送 `codex/dav-4-p2a-trunk`，交付只能推到各自的 agent 分支，并报告分支名与完整 SHA。主干只由总控执行，或由总工按总控签字评论中写明的完整 SHA 快进。
  2. 派工与规格中不得再用“推远端”这类含糊说法，一律写“推到你的 agent 分支，不得推主干”。
  3. 调度助手核收任何交付时，都要比对 `git ls-remote origin codex/dav-4-p2a-trunk`；主干 tip 出现不在签字清单上的 SHA，立即上报总工与总控。总工的监视器同样加上这一项。
  4. 部署只认签字评论中写明的完整 SHA。
- **技术拦截（待 David 决定）：** 所有执行者都用同一个 GitHub 账号推送，GitHub 分支保护无法区分推送者，所以用不上。可选方案是本机全局 git pre-push 钩子：拒绝推送到 `refs/heads/codex/dav-4-p2a-trunk`，除非显式设置了总控放行变量。这会改动本机的全局 git 配置，须 David 同意后才能实施。

### D-049：防重复计入守卫掏空采纳账本；更正 D-047 对 DAV-1311 的归因；DAV-1320 暂缓、DAV-1322 合入（有效，2026-09-27）

- **发现（总控在核对 DAV-1320 的翻转样本时查出）：** `apply_manager_double_count_guard` 把 `cluster_id`（E-01 证据簇）当成“同一事件”的标识，每个簇只保留第一条论点，其余全部剔出 adopted，不区分多空立场。09-25 以后 168 个档位全部受影响，每档剔除 4～16 条（众数 8）。研究经理的采纳账本普遍被掏空，direction_basis 大量为 unledgered / partial_only。守卫本意只针对 expectation_revision 为 accounted_for / unknown 时的同一预告或公告事件。立施工卡 DAV-1338（urgent）。
- **更正 D-047：** D-047 认为 DAV-1311 的“全额采纳含未核实”类误拦源于“一致性检查跑在守卫裁剪前的旧账本上”，这只是表象，根因是守卫过度剔除。D-047 中基于该归因的裁决作以下修正：
  - DAV-1320（候选 `9029e3a9`）**暂不合入**：按原规格重算，会把账本已被掏空的裁决放行成 VALID。例如 af25901e:short_term 翻转后结论看空，但账本里没有看空论点，属于假 clean。DAV-1338 合入后，重新测量过期失败项，再定 1320 的去留。
  - DAV-1311 各类误拦的比例，在 DAV-1338 上线后重新测算，然后再立账本归类与 E-04 规则的卡。
- **DAV-1322：** 准予合入 `ae762eed`（已快进），随下一批部署。DAV-1338 上线之前，H1b 各 cohort 的数字建立在被污染的账本上，只作参考。
- **总控教训：** 立“修表象”的卡之前，必须先核对上游数据（这次是守卫实际剔除了什么）是否合理。规格前提未经实测，不得派工。

### D-050：四卡同批放行（1338 / 1343 / 1321 / 1322）；确认规则语义修正；1321 净改动压缩整合（有效，2026-09-27）

- **DAV-1338 重放口径更正：** 首轮以“落库 adopted ∪ 旧守卫 excluded”重建守卫前账本，把研究经理明确否决的论点塞回了 adopted，制造了假冲突。返工改用原始 MANAGER_VERDICT 机读块；09-25 以后的子集一致性失败项与落库逐字一致，口径自证成立。复审抽样要求（D-048 附）自此写入派工模板。
- **确认规则语义修正（DAV-1343）：** 焦点论点（core claims）若在研究经理最终账本的 rejected 里、且核验结论不是 adopt，视为“已裁决、零贡献”，不再使确认状态成为 UNRESOLVED。理由：研究经理正确否决未核实的争议论点，不应导致整份裁决只能观望。核验为 adopt 却被否决的论点，仍由一致性检查问责。修正前，DAV-1338 单独上线会有 48 档正确裁决被降为 WAIT。
- **DAV-1321 整合：** 候选分支中途提交过、后又删除约 90 万行含报告原句的重放 JSON。为避免进入公开仓库历史，总控取净改动压成单提交 `56807dc`，净 patch-id 与候选一致。R2（字段级口径混用连坐）打击面过宽，另立 DAV-1346，作为下一批第一优先。
- **放行：** 准予部署 `56807dc7ec50fbfaabe3a569066620bcff4a7147`，整合后全量回归 5764 通过、0 失败。上线后总控审计前 10 份真实报告；回滚触发条件为假 clean、ABSTAIN 比例显著上升、新的入库失败。
- **预期影响（告知 David）：** 旧守卫把未核实的焦点论点剔出账本，一部分买卖建议是因此才被放行的。修正后，这部分会如实转为观望或弃权；同时有 45 档过去被错误压成观望的裁决恢复为可执行。
