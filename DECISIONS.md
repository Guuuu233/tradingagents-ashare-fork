...
- 本决定只允许显示文本和显示颜色查找的变更，不改持久化方向值、业务决策、报告内容或 API。候选
  已以 `--ff-only` 线性合入目标主线；本决定不触发部署、重启、前端 live bundle 更新、生产库写入、
  真实分析或真实社交采集。线上仍运行 `6cc4e152...`，后续发布须另走发布门并重新构建/复验 bundle。

### 当前不变的原则

- D-009 的状态拆分、PIT、证据独立性和统计排除原则仍有效；D-012 的红队完备性、逐条实跑和 RT-FULL 仍是行为类候选的硬门槛。
- 当前施工主干当前 tip 为 DAV-914 `8ccecb8d59de31835f9d0f67578123db9a358e7b`，并包含 P1-G 候选 `054a76210f799029fe8d390512c763c36f0fb585`、P1-F 候选
