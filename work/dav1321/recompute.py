"""DAV-1321 零 LLM 重放：price_ref 提取器精度修正的验收重放。
...
用法（仓库根目录，锁定解释器）：
    env -u PYTHONPATH /Users/davidliu/Documents/TradingAgents-AShare/.venv310/bin/python \
        work/dav1321/recompute.py --impl baseline|candidate

baseline = `git show 628f25107c77c72f3957ba0e67b1fd136cd6b4b4:tradingagents/agents/utils/price_ref_registry.py`
（派工基线 tip）；candidate = 工作区实现。gate 两侧共用主干 evaluate_price_basis_gate
（本卡不改 gate 判定规则）。

输出：work/dav1321/out/{impl}.json；candidate 跑时额外生成 diff 汇总
（翻转为通过/新增拦截档位、逐 violation kind 计数、反向指标=候选丢失的
vendor_qfq/raw/pit_raw ref 清单）。
"""
...
BASELINE_GIT_PATH = (
    "628f25107c77c72f3957ba0e67b1fd136cd6b4b4"
