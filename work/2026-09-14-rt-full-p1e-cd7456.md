# P1-E 财务披露日 PIT：同口径全量回归证据

日期：2026-09-14（Australia/Perth）

## 1. 验证对象

- 基线：`9d702e7522c94bf3ac983cb1ede10943cfca1a4b`（当前线上业务版本）。
- 候选：`cd7456012fe2e0b03bd33333e6972301c1c76ddc`，直接父为
  `251fd00aa8a9584850cae5d4ab5adfbd3c5d3b94`。
- 候选白名单：
  `tradingagents/dataflows/providers/cn_fuyao_provider.py`、
  `tests/test_cn_fuyao_provider.py`。
- 两个版本分别位于独立、干净的 detached worktree；使用同一台机器、同一 Python 3.10
  解释器，并以 `env -u PYTHONPATH` 清除外部路径污染。
- 唯一全量命令：

  ```text
  env -u PYTHONPATH /Users/davidliu/Documents/TradingAgents-AShare/.venv310/bin/python -m pytest -q
  ```

## 2. 全量结果

| 版本 | 结果 | 用时 |
|---|---|---:|
| 基线 `9d702e7` | `18 failed, 4415 passed, 1 skipped, 3 deselected, 176 warnings` | 1926.71s |
| 候选 `cd74560` | `18 failed, 4426 passed, 1 skipped, 3 deselected, 176 warnings` | 1107.17s |

原始输出保留在：

- `/private/tmp/ta-p1e-rt-baseline-9d-20260914.log`
- `/private/tmp/ta-p1e-rt-candidate-cd7456-20260914.log`

候选多通过 11 项；失败数没有增加。

## 3. 失败集合逐项比对

两边失败节点的双向差集均为空：

```text
comm -23 /private/tmp/ta-p1e-failed-base.txt /private/tmp/ta-p1e-failed-cand.txt  => empty
comm -13 /private/tmp/ta-p1e-failed-base.txt /private/tmp/ta-p1e-failed-cand.txt  => empty
```

共同的 18 项为：

```text
tests/test_dav27_report_semantics.py::test_ordinary_and_streaming_paths_merge_gaps_and_keep_graph_fallback[False]
tests/test_dav27_report_semantics.py::test_ordinary_and_streaming_paths_merge_gaps_and_keep_graph_fallback[True]
tests/test_debate_state_persistence.py::TestJobExecutionDebatePersistence::test_dual_horizon_persists_debate_states_to_result_data
tests/test_debate_state_persistence.py::TestJobExecutionDebatePersistence::test_intent_hoist_persists_debate_states_to_result_data
tests/test_debate_state_persistence.py::TestJobExecutionDebatePersistence::test_single_horizon_persists_debate_states_to_result_data
tests/test_debate_state_persistence.py::TestJobExecutionDebatePersistence::test_job_execution_with_legitimate_empty_probability_persists_note_and_4_of_4_metrics
tests/test_debate_state_persistence.py::TestJobExecutionDebatePersistence::test_job_execution_hold_decision_persists_note_and_metrics_consistency
tests/test_h1b_gates.py::TestH1bV2OnlySampleFilteringAndIndustry::test_verify_h1b_gates_script_runs_and_verifies_v2_only
tests/test_provider_date_guards.py::test_all_time_sensitive_get_methods_have_date_param
tests/test_recalculate_weekly_metrics.py::TestCliIntegrationAndSubprocess::test_cli_subprocess_format_json
tests/test_signal_processing.py::test_negated_buy_phrase_is_not_buy
tests/test_signal_processing.py::test_negated_build_position_is_not_buy
tests/test_signal_processing.py::test_plain_hold_is_hold
tests/test_social_data_api.py::test_status_proves_four_dimensions_independent
tests/test_two_stage_analyst_topology.py::test_full_7_analysts_two_stage_topology
tests/test_two_stage_analyst_topology.py::test_phase1_only_topology
tests/test_two_stage_analyst_topology.py::test_phase2_only_topology
tests/test_two_stage_analyst_topology.py::test_mixed_subset_topology
```

## 4. 审查与合入状态

- DAV-903 已由**代码审核员**对完整候选 SHA
  `cd7456012fe2e0b03bd33333e6972301c1c76ddc` 完成只读审查并 PASS。
- 审查确认直接父、两文件白名单、clean worktree 和 PIT fail-closed 语义；固定 Python
  3.10 的专项证据为 `91 passed`，关联集合为 `295 passed, 3 deselected`。
- 候选已在集成树中从 `251fd00aa8a9584850cae5d4ab5adfbd3c5d3b94` 线性快进到
  `cd7456012fe2e0b03bd33333e6972301c1c76ddc`；本证据对应的治理文档提交为该代码候选的
  线性后代。

## 5. 运行边界

- 证据生成只读测试代码和隔离 worktree；没有调用真实分析入口。
- 线上服务在回归期间仍运行 `9d702e7522c94bf3ac983cb1ede10943cfca1a4b`，P1-E
  当时尚未部署。
- 生产 SQLite 未被测试写入；部署前后仍须重新执行备份、`quick_check`、reports 计数、
  `/healthz` 完整 SHA 和只读烟测。
- 本证据只证明候选相对基线没有新增全量回归，不证明生产财务数据已经按披露日重新采集，
  也不授权真实采集、信用加权、历史重写或生产业务写入。

## 6. 受控发布回读

- 发布版本为治理文档线性后代 `026349614a3f1b92a95dc06c0515f10ebec193bc`，其代码父为
  `cd7456012fe2e0b03bd33333e6972301c1c76ddc`；发布副本为
  `/private/tmp/ta-release-p1e-0263496-20260914`。
- 旧 8000 服务已正常停止；新服务 PID `19213` 在 8000 运行，工作目录与发布副本一致。
- `/healthz` 返回 `status=ok`，完整 `commit_sha` 和 `build_identity` 均为
  `026349614a3f1b92a95dc06c0515f10ebec193bc`。
- 只读烟测通过：provider health 为 `ok`；600519.SH 的 2026-09-08 至 09-09 K 线返回两行；
  social-data 为 `disabled`。
- 发布前后的生产库仍为 285339648 bytes、SHA-256
  `94d2f6740db4f2065100479dd5cb3ccf5d8a504447a55fa8f19635927ce83010`，
  `quick_check=ok`，reports 为 `1409/793/616`。发布备份
  `work/tradingagents.db.bak-20260914-predeploy-0263496` 也为 `quick_check=ok`、
  reports `1409/793/616`；该备份使用 SQLite 一致性备份，字节 SHA 为
  `df628f4293069a820ef69a136a1acbb0e7a517f6638dd24a9faeea30f5b51b9e`。
- 未调用真实分析入口，因此没有新增生产报告、trace 或 PIT 业务样本；本次发布不改变既有
  真实采集、信用加权、历史重写和生产写入红线。
