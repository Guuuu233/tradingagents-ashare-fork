#!/usr/bin/env bash
# Deploy H1b fix 4b540b0 to local 8000.
...
SHA="4b540b0c9b08d77a12ce7d08cfdf0095288cd350"
SERVE="/private/tmp/ta-serve-4b540b0"
PY="$REPO/.venv310/bin/python"
DB="$REPO/data/tradingagents.db"
FRONT_DIST="$REPO/frontend/dist"
TS="$(date +%Y%m%d-%H%M%S)"
LOG="/private/tmp/ta-serve-8000-${TS}-4b540b0.log"
BAK="$REPO/work/tradingagents.db.bak-${TS}-deploy-4b540b0"
...
CURRENT_PID=$(lsof -tiTCP:8000 -sTCP:LISTEN 2>/dev/null | head -1 || true)
if [ -n "$CURRENT_PID" ]; then
  ps eww -p "$CURRENT_PID" -o command= | tr ' ' '\n' | grep -E '^TA_SOCIAL_(MODE|PLATFORMS|ARCHIVE_DB)=' > "$SOCIAL_ENV_SNAPSHOT" || true
else
  : > "$SOCIAL_ENV_SNAPSHOT"
fi
SOCIAL_MODE=$(awk -F= '$1=="TA_SOCIAL_MODE"{print substr($0,index($0,"=")+1)}' "$SOCIAL_ENV_SNAPSHOT")
SOCIAL_PLATFORMS=$(awk -F= '$1=="TA_SOCIAL_PLATFORMS"{print substr($0,index($0,"=")+1)}' "$SOCIAL_ENV_SNAPSHOT")
SOCIAL_ARCHIVE_DB=$(awk -F= '$1=="TA_SOCIAL_ARCHIVE_DB"{print substr($0,index($0,"=")+1)}' "$SOCIAL_ENV_SNAPSHOT")
...
NEW_PID=$(lsof -tiTCP:8000 -sTCP:LISTEN | head -1)
NEW_SOCIAL=$(ps eww -p "$NEW_PID" -o command= | tr ' ' '\n' | grep -E '^TA_SOCIAL_(MODE|PLATFORMS|ARCHIVE_DB)=' || true)
echo "$NEW_SOCIAL"
echo "$NEW_SOCIAL" | grep -q '^TA_SOCIAL_MODE=shadow$' || fail "deployed TA_SOCIAL_MODE is not shadow"
echo "$NEW_SOCIAL" | grep -q '^TA_SOCIAL_PLATFORMS=xhs,dy$' || fail "deployed TA_SOCIAL_PLATFORMS is not xhs,dy"
echo "$NEW_SOCIAL" | grep -q '^TA_SOCIAL_ARCHIVE_DB=' || fail "deployed TA_SOCIAL_ARCHIVE_DB missing"
