"""DAV-1246 零 LLM 重算：5909db5 基线 vs 候选（price-ref 精度收紧）。

用法（仓库根目录，锁定解释器）：
    env -u PYTHONPATH /Users/davidliu/Documents/TradingAgents-AShare/.venv310/bin/python \
        work/dav1246/recompute.py --impl baseline|candidate
...
baseline 实现 = 运行时 `git show 5909db5:tradingagents/agents/utils/price_ref_registry.py`
（D-040/D-036：不提交整份基线拷贝；out/*.json 不入库，仅在汇总记 SHA256）。
gate 两版共用 trunk evaluate_price_basis_gate（本卡不改 gate 判定规则）。

输出：work/dav1246/out/{impl}.json + diff SUMMARY（candidate 跑时生成）。
"""
...
BASELINE_GIT_PATH = "5909db5:tradingagents/agents/utils/price_ref_registry.py"
...
def load_impl(name: str):
    ...
    m = types.ModuleType("registry_5909db5")
    sys.modules["registry_5909db5"] = m  # dataclass 处理需要模块已注册
