...
- **`/limit-up-ladder` 未接线**：`cn_fuyao` 尚未实现独立连板天梯能力；「连板天梯」语义仅由
  `get_zt_pool` 返回中的「连板分布」部分覆盖。P1-F 设计已冻结在
  `work/2026-09-14-p1f-limit-up-ladder-design.md`，在实施、代码审核、RT-FULL 和发布前，
  文档不得宣称「连板天梯已覆盖」。该上游端点没有日期参数，历史分析必须 fail-closed，且
  不得从涨停池合成或自动回退到其他来源。
