...
状态：**凭据侧已闭环**（2026-09-20 用户手动轮换 CPA key，现有 key 全部作废）。
历史侧仍未处理：旧 key 的明文仍在公开 `origin/main` 历史中，但已失效，故不再构成可用凭据。
...
| 是否已吊销 | **是**。2026-09-20 用户在 CPA 侧手动轮换，现有 key 全部作废，见 §4 |
...
用户指出该凭据曾在 2026-09-20 会话中贴出。值不记录。若与 CPA 同源，已随本次全量轮换作废；若属独立签发方，仍需单独撤销或轮换确认。
...
**该探测当时不足以证明吊销。** 后续由用户在 CPA 服务端手动轮换密钥并确认现有 key 全部作废（2026-09-20），
吊销以该操作为准，而非以本表的 401 为准 —— 记此区分，避免日后把无判别力的探测当成证据复用。
...
2. 旧 key 已在公开 `origin/main` 历史中，不得再称「未上传」。**2026-09-20 已在 CPA 侧作废**，历史里留存的是一个死 key。
